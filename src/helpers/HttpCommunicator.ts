...
import { getItem } from "./LocalStorageUtility";
...
export default class HttpCommunicator implements IHttpCommunicator {
  ...
  getWithBody<TRequest, TResponse>(
    path: string,
    body: TRequest
  ): Promise<TResponse> {
  ...
  private async fetchAsync<TRequest, TResponse>(
    method: string,
    path: string,
    body?: TRequest | unknown
  ): Promise<TResponse> {
  ...
  private getHeaders(): Headers {
    ...
    if (user)
      headers.append("Authorization", `Bearer ${user.auth?.access_token}`);
    return headers;
  }
}
